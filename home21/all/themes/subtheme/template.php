...
function subtheme_css_alter(&$css) {
  ...
  // Remove subtheme stylesheets if it is not the default theme.
  if ($active_theme != 'subtheme') {
    unset($css[drupal_get_path('theme', 'subtheme') . '/assets/css/subtheme.style.css']);
  }
...
function subtheme_js_alter(&$javascript) {
  // Add subtheme-modal when required.
  if (module_exists('ctools')) {
    $ctools_modal = drupal_get_path('module', 'ctools') . '/js/modal.js';
    $subtheme_modal = drupal_get_path('theme', 'subtheme') . '/assets/js/subtheme.modal.js';
    if (!empty($javascript[$ctools_modal]) && empty($javascript[$subtheme_modal])) {
      $javascript[$subtheme_modal] = array_merge(
        drupal_js_defaults(), array('group' => JS_THEME, 'data' => $subtheme_modal));
    }
  }

  // Add subtheme-field-slideshow when required.
  if (module_exists('field_slideshow')) {
    $field_slideshow = drupal_get_path('module', 'field_slideshow') . '/field_slideshow.js';
    $subtheme_field_slideshow = drupal_get_path('theme', 'subtheme') . '/assets/js/subtheme.slideshow.js';
    if (!empty($javascript[$field_slideshow]) && empty($javascript[$subtheme_field_slideshow])) {
      $javascript[$subtheme_field_slideshow] = array_merge(
        drupal_js_defaults(), array('group' => JS_THEME, 'data' => $subtheme_field_slideshow));
    }
  }

  // Add subtheme-progress when required.
  $progress = 'misc/progress.js';
  $subtheme_progress = drupal_get_path('theme', 'subtheme') . '/assets/js/subtheme.progress.js';
  if (!empty($javascript[$progress]) && empty($javascript[$subtheme_progress])) {
    $javascript[$subtheme_progress] = array_merge(
      drupal_js_defaults(), array('group' => JS_THEME, 'data' => $subtheme_progress));
  }
}
...
function subtheme_preprocess_page(&$variables) {
  ...
  $variables['main_menu'] = _subtheme_dropdown_menu_tree(variable_get('menu_main_links_source', 'main-menu'), $main_menu_parameters);
