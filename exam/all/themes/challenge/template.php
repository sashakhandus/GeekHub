 ...
 */
function challenge_preprocess_html(&$variables) {
  global $base_url;
 ...
 */
function challenge_css_alter(&$css) {
  ...
  // Remove challenge stylesheets if it is not the default theme.
  if ($active_theme != 'challenge') {
    unset($css[drupal_get_path('theme', 'challenge') . '/assets/css/challenge.style.css']);
  }
 ...
 */
function challenge_js_alter(&$javascript) {
  // Add challenge-modal when required.
  if (module_exists('ctools')) {
    $ctools_modal = drupal_get_path('module', 'ctools') . '/js/modal.js';
    $challenge_modal = drupal_get_path('theme', 'challenge') . '/assets/js/challenge.modal.js';
    if (!empty($javascript[$ctools_modal]) && empty($javascript[$challenge_modal])) {
      $javascript[$challenge_modal] = array_merge(
        drupal_js_defaults(), array('group' => JS_THEME, 'data' => $challenge_modal));
    }
  }

  // Add challenge-field-slideshow when required.
  if (module_exists('field_slideshow')) {
    $field_slideshow = drupal_get_path('module', 'field_slideshow') . '/field_slideshow.js';
    $challenge_field_slideshow = drupal_get_path('theme', 'challenge') . '/assets/js/challenge.slideshow.js';
    if (!empty($javascript[$field_slideshow]) && empty($javascript[$challenge_field_slideshow])) {
      $javascript[$challenge_field_slideshow] = array_merge(
        drupal_js_defaults(), array('group' => JS_THEME, 'data' => $challenge_field_slideshow));
    }
  }

  // Add challenge-progress when required.
  $progress = 'misc/progress.js';
  $challenge_progress = drupal_get_path('theme', 'challenge') . '/assets/js/challenge.progress.js';
  if (!empty($javascript[$progress]) && empty($javascript[$challenge_progress])) {
    $javascript[$challenge_progress] = array_merge(
      drupal_js_defaults(), array('group' => JS_THEME, 'data' => $challenge_progress));
  }
}
...
function challenge_preprocess_page(&$variables) {
  ...
  $variables['main_menu'] = _challenge_dropdown_menu_tree(variable_get('menu_main_links_source', 'main-menu'), $main_menu_parameters);
